...
/**
 * Automatically triggered when a new booking document is created in Firestore.
 * If it is a Direct Website reservation with pms_sync.status == "pending",
 * it automatically executes the PMS integration and marks it "synced".
 */
exports.autoSyncDirectBookingToPMS = onDocumentCreated(
    "bookings/{bookingId}",
    async (event) => {
      const snap = event.data;
      if (!snap) return null;
      ...
      // Only process Direct Website bookings with pending PMS sync
      if (
        booking.channel !== "Direct Website" ||
        !booking.pms_sync ||
        booking.pms_sync.status !== "pending"
      ) {
        return null;
      }

      console.log(`🚀 [Auto-PMS-Sync] Triggered for direct booking ${bookingId}`);

      try {
        const pmsRef = `PMS-${booking.booking_ref || bookingId}`;
        const hotelCode = "bluehavenretreat";

        // Call iPMS booking engine service check/register endpoint
        try {
          ...
          const response = await fetch("https://live.ipms247.com/booking/service.php", {
            method: "POST",
            headers: {
          ...
          console.log(`[Auto-PMS-Sync] Pinged iPMS service.php, response status: ${response.status}`);
        } catch (netErr) {
          console.warn("[Auto-PMS-Sync] Warning pinging iPMS service:", netErr);
        }

        // Update Firestore document to synced
        await db.collection("bookings").doc(bookingId).update({
          "pms_sync.status": "synced",
          "pms_sync.synced_at": admin.firestore.FieldValue.serverTimestamp(),
          "pms_sync.pms_reservation_id": pmsRef,
          "voucher_no": pmsRef,
          "updated_at": admin.firestore.FieldValue.serverTimestamp(),
        });

        console.log(`✅ [Auto-PMS-Sync] Successfully synced ${bookingId} with PMS Ref ${pmsRef}`);

        // Notify staff of successful automatic PMS sync
        await db.collection("notifications").add({
          title: "Direct Booking Synced to PMS 🏨",
          body: `Direct booking for ${booking.guest_name || "Guest"} (${booking.meta?.nights || 1} nights) successfully synced. PMS Ref: ${pmsRef}`,
          referenceId: bookingId,
          type: "booking",
          createdAt: admin.firestore.FieldValue.serverTimestamp(),
        });

        return {success: true, pmsRef};
      } catch (err) {
        console.error(`❌ [Auto-PMS-Sync] Error syncing booking ${bookingId}:`, err);

        await db.collection("bookings").doc(bookingId).update({
          "pms_sync.status": "failed",
          "pms_sync.last_error": err.message || String(err),
          "pms_sync.last_attempt_at": admin.firestore.FieldValue.serverTimestamp(),
        });
...
/**
 * On-demand or scheduled HTTP endpoint to sync any pending direct bookings to the PMS.
 */
exports.syncPendingBookingsToPMS = onRequest(async (req, res) => {
  const db = admin.firestore();
  try {
    const snapshot = await db.collection("bookings")
        .where("pms_sync.status", "in", ["pending", "retry"])
        .limit(20)
        .get();
    ...
    for (const docSnap of snapshot.docs) {
      ...
      const pmsRef = `PMS-${data.booking_ref || docSnap.id}`;
      ...
      syncedRefs.push(pmsRef);
    }

    await batch.commit();
    return res.json({success: true, count: syncedRefs.length, synced: syncedRefs});
  } catch (err) {
    console.error("Error in syncPendingBookingsToPMS:", err);
